...
# 데이터 유효성 검사
if data is not None:
    # 날짜 및 기온 컬럼 이름 설정 (예상되는 데이터 컬럼: 날짜, 최저기온(℃))
    if '날짜' not in data.columns or '최저기온(℃)' not in data.columns:
        st.error("데이터에 '날짜' 또는 '최저기온(℃)' 컬럼이 없습니다.")
    else:
        # 날짜 컬럼을 datetime 형식으로 변환
        data['날짜'] = pd.to_datetime(data['날짜'], errors='coerce')
        ...
        # 연도별 최고기온 계산
        data['Year'] = data['날짜'].dt.year
        yearly_min_temp = data.groupby('Year')['최저기온(℃)'].min()
        
        # 그래프 생성
        st.subheader("연도별 최저기온 그래프")
        fig, ax = plt.subplots()
        yearly_min_temp.plot(kind='line', marker='o', ax=ax)
        ax.set_title("연도별 최저기온")
        ax.set_xlabel("연도")
        ax.set_ylabel("기온 (°C)")
        plt.grid()
